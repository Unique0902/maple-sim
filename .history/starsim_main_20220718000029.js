...
function reinforceStarforce(userItem) {
  const successNum = calculateStarforceSuccess(userItem);
  const destroyedNum = calculateStarforceDestroy(userItem);
  const randomNum = Math.random();
  console.log(randomNum);
  if (randomNum < successNum) {
    userItem.plusStar();
  } else if (randomNum >= successNum && randomNum < successNum + destroyedNum) {
    userItem.destroy();
  } else {
    const starNum = userItem.returnStarNum;
    if (
