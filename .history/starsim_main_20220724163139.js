...
let additionClickEvent;
let lastWindowX;
let lastWindowY;

additionBoxDescriptionBlock.addEventListener(
  'mousedown',
  (additionClickEvent = () => {
    const additionFirstRect = reinforceAdditionBox.getBoundingClientRect();
    additionFirstLeft = additionFirstRect.left;
    additionFirstTop = additionFirstRect.top;
    additionBoxDescriptionBlock.removeEventListener(
      'mousedown',
      additionClickEvent
    );
  })
);
let additionFirstLeft;
let additionFirstTop;
...
additionBoxDescriptionBlock.addEventListener('mousedown', (e) => {
  ...
  const leftDiff = additionleft - additionFirstLeft;
  const topDiff = additiontop - additionFirstTop;
...
document.addEventListener('mouseup', () => {
  ...
  additionMouseMoveEvent.style.transform = `translate(${lastWindowX}px,${lastWindowY}px)`;
  resultMouseMoveEvent.style.transform = `translate(${lastWindowX}px,${lastWindowY}px)`;
}); // 리팩토링 필요
...
resultBoxMainBlock.addEventListener(
  'mousedown',
  (resultClickEvent = () => {
    const resultFirstRect = reinforceResultBox.getBoundingClientRect();
    resultFirstLeft = resultFirstRect.left;
    resultFirstTop = resultFirstRect.top;
    resultBoxMainBlock.removeEventListener('mousedown', resultClickEvent);
  })
);
let resultFirstLeft;
let resultFirstTop;
...
resultBoxMainBlock.addEventListener('mousedown', (e) => {
  ...
  const leftDiff = resultleft - resultFirstLeft;
  const topDiff = resulttop - resultFirstTop;
  document.removeEventListener('mousemove', resultMouseMoveEvent);
  document.addEventListener(
    'mousemove',
