...
additionBoxDescriptionBlock.addEventListener('mousedown', (e) => {
  firstCurXLoc = e.clientX;
  firstCurYLoc = e.clientY;
  const additionRect = additionBoxDescriptionBlock.getBoundingClientRect();
  const additionBoxLeft = additionRect.left;
  const additionBoxTop = additionRect.top;
  additionBoxDescriptionBlock.addEventListener(
    'mousemove',
    (additionMouseMoveEvent = (e) => {
      const x = e.clientX;
      const y = e.clientY;
      const xDiff = firstCurXLoc - additionBoxLeft;
      const yDiff = firstCurYLoc - additionBoxTop;
      reinforceAdditionBox.style.transform = `translate(${
        x - xDiff - additionBoxLeft
      }px,${y - yDiff - additionBoxTop}px)`;
    })
  );
});
additionBoxDescriptionBlock.addEventListener('mouseup', () => {
  additionBoxDescriptionBlock.removeEventListener(
    'mousemove',
    additionMouseMoveEvent
  );
});
