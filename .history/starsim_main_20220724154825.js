...
additionBoxDescriptionBlock.addEventListener('mousedown', (e) => {
  ...
  const reinforceWindowRect = reinforceWindow.getBoundingClientRect();
  const additionBoxLeft = additionRect.left + reinforceWindowRect.left;
  const additionBoxTop = additionRect.top + reinforceWindowRect.top;
  const xDiff = firstCurXLoc - additionBoxLeft;
  const yDiff = firstCurYLoc - additionBoxTop;
  console.log(additionRect.top);
  console.log(reinforceWindowRect.top);
  ...
  document.addEventListener(
    ...
    (additionMouseMoveEvent = (e) => {
      const x = e.clientX;
      const y = e.clientY;
      reinforceAdditionBox.style.transform = `translate(${x - firstCurXLoc}px,${
        y - firstCurYLoc
      }px)`;
    })
  );
