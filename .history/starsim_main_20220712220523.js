...
function updateSearchedItemPage() {
  ...
  if (clothBtn.classList.contains('selected')) {
    ...
    console.log(pageInformBlock);
    pageInformBlock.innerHTML(
      `1 / ${(filteredClothItemArr.length - 1) / 10 + 1}`
    );
    ...
    const filteredWeaponItemArr = filterWeaponItems();
    pageInformBlock.innerHTML(
      `1 / ${(filteredWeaponItemArr.length - 1) / 10 + 1}`
    );
