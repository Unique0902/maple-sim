...
function applySoundSettings() {
  normalSoundsArr.forEach((x) => (x.volume = systemSetting.sound.normal / 100));
  bgmSoundsArr.forEach((x) => (x.volume = systemSetting.sound.bgm / 100));
  monsterSoundsArr.forEach(
    (x) => (x.volume = systemSetting.sound.monster / 100)
  );
  skillSoundsArr.forEach((x) => (x.volume = systemSetting.sound.skill / 100));
  voiceSoundsArr.forEach((x) => (x.volume = systemSetting.sound.voice / 100));
  soundsArr.forEach((x) =>
    x.forEach((y) => (y.volume *= systemSetting.sound.master / 100))
  );
}
