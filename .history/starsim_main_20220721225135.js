...
let recentReinforceTime;

function checkStarcatchLevel(userItem) {
  const nowTime = new Date();
  const nowTimeMin = Math.floor(nowTime.getTime() / 60000);
  if (recentReinforceTime) {
    const recentTimeMin = Math.floor(recentReinforceTime.getTime() / 60000);
    const diffMin = nowTimeMin - recentTimeMin;
    console.log(nowTimeMin, recentTimeMin, diffMin);
    if (diffMin >= 40) {
      userItem.starcatchLevel = 1;
      userItem.clearStarforceNum();
      console.log('hi');
    } else if (diffMin >= 10) {
      for (let i = nowTimeMin; i <= recentTimeMin; i++) {
        if (i % 10 === 0) {
  ...
  console.log(userItem.returnStarforceNum());
  userItem.checkPlusStarcatchLev();
  console.log(userItem.starcatchLevel);
  recentReinforceTime = nowTime;
}
