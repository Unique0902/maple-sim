...
class WeaponItem {
  constructor(name, handNum, weaponType, level, attackPower, weaponConstant, attackSpeed) {
    this.name = name;
    ...
    this.weaponConstant = weaponConstant;
    this.attackSpeed = attackSpeed;
    if (
      this.weaponType === WeaponType.magicGuntlet ||
      this.weaponType === WeaponType.wand ||
      this.weaponType === WeaponType.staff ||
      this.weaponType === WeaponType.shiningRoad ||
      this.weaponType === WeaponType.espLimiter
    ) {
...
function updateReinforceDescription(userItem) {
  ...
  const power = calculateStarforceAttackPower(userItem);
  if (starNum <= 10) {
    if (attackPower === null) {
