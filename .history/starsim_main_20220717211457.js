...
function showDestroyedText() {
  if (destroyedText.classList.contains('none')) {
    destroyPerText.classList.remove('none');
  }
}
function hideDestroyedText() {
  if (!destroyedText.classList.contains('none')) {
    destroyPerText.classList.add('none');
  }
}
...
function updateStatText(job, stat) {
  showStrText();
  showDexText();
  showIntText();
  showLukText();
...
function updateReinforceDescriptionText(userItem, level, starNum) {
...
function updateReinforceDescription(userItem) {
  ...
  if (starNum >= 0 && starNum <= 24) {
    ...
    } else if (level >= 138 && level <= 200) {
      if (starNum <= 24) {
      }
    }
  } else {
    throw new Error('you can not reinforce starforce.');
  }
  ...
  if (starNum <= 10) {
    ...
    실패(<span class="youji">유지</span>)확률: ${100 - successPer}.0%<br>
    <br>
    STR: +${stat}<br>
    DEX +${stat}<br>
    INT +${stat}<br>
