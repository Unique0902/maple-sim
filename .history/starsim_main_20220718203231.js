...
const jumpStat = document.querySelector('#jumpStat');
...
function updateEtcStatText(userItem, type, maxStat, defenseStat, speedStat) {
  if (maxStat != null) {
    showMaxHpText();
    showMaxMpText();
    if (type === 'cloth') {
      hideMaxMpText();
      maxHpText.innerText = `${maxStat}`;
    } else if (type === 'weapon') {
      maxHpText.innerText = `${maxStat}`;
      maxMpText.innerText = `${maxStat}`;
    }
  }
}
...
function updateReinforceDescriptionText(userItem, starNum) {
  ...
  updateEtcStatText(userItem, type, maxStat, defenseStat, speedStat);
}
