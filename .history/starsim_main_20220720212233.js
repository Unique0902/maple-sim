...
function reinforceStarforce(userItem) {
  ...
  if (userItem.isStarcatch === true && userItem.isStarcatchSuccess === true) {
    successNum *= 1.05;
  }
...
function checkStarcatchResult() {
  const left = parseFloat(starcatchStar.style.left);
  const min = starcatchTarget.getBoundingClientRect;
  // if(left <= )
}
console.log(
  document.querySelector('.starAnimation__star').getBoundingClientRect().left
);
...
function startStarcatch(userItem) {
  ...
  console.log(optionBtnBlock.getBoundingClientRect.left);
  setTimeout(() => {
    showElem(starAnimation);
    starcatchStopBtn.addEventListener('click', () => {
      hideElem(starcatchCountAnimation);
      hideElem(starAnimation);
      showElem(starcatchCircleAnimation);
      checkStarcatchResult();
...
function checkStarcatch(userItem) {
  if (userItem.checkChanceTime === true) {
    ...
    if (starcatchCheckBox.checked) {
      ...
      userItem.isStarcatch = true;
    }
  }
}
function reinforceonstarcatch() {}
