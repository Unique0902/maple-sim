...
itemWindowTable.addEventListener('click', (e) => {
  if (e.target.className === 'table__img') {
    if (e.target.dataset.type === 'cloth') {
      ...
      followedItemBox.setAttribute('data-type', e.target.dataset.type);
      followedItemBox.setAttribute('data-id', e.target.dataset.id);
      followedItemBox.setAttribute(
        'data-reinforceid',
        e.target.dataset.reinforceid
      );
    ...
    document.addEventListener(
      ...
      function mousemove(e) {
        moveFollowedItemBox(e.pageX - 15, e.pageY - 15);
