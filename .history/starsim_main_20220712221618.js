...
let filteredClothItemArr;
let filteredWeaponItemArr;
...
function filterClothItems() {
  ...
  filteredClothItemArr = clothItemArr.filter(
    (x) =>
...
function filterWeaponItems() {
  ...
  const frontLevel = translateLevelLimit(levelFrontLimitBlock.value);
  const backLevel = translateLevelLimit(levelBackLimitBlock.value);
  filteredWeaponItemArr = weaponItemArr.filter(
    (x) =>
...
function updateSearchedItemPage() {
  ...
  if (clothBtn.classList.contains('selected')) {
    filterClothItems();
    pageInformBlock.innerHTML = `1 / ${
      parseInt((filteredClothItemArr.length - 1) / 10) + 1
    }`;
    ...
    filterWeaponItems();
    pageInformBlock.innerHTML = `1 / ${
      parseInt((filteredWeaponItemArr.length - 1) / 10) + 1
    }`;
    filteredWeaponItemArr.forEach((item) => {
