...
function addClothItemInTable(id) {
  const item = clothItemArr.find((x) => x.id === parseInt(id));
  const imgElem = makeItemImgElem(item);
  const td = document.querySelector(
    `#tr${parseInt(tableItemNum / 4) + 1} #td${(tableItemNum % 4) + 1}`
  );
}

function addWeaponItemInTable(id) {
  const item = weaponItemArr.find((x) => x.id === parseInt(id));
}
...
itemTable.addEventListener('click', (e) => {
  if (e.target.className === 'itemRow__addItemBtn') {
    ...
    if (itemRowBlock.dataset.type === 'cloth') {
      tableItemNum++;
      if (tableItemNum > 24) {
        return;
      }
      addClothItemInTable(itemRowBlock.dataset.id);
    } else {
      tableItemNum++;
      if (tableItemNum > 24) {
        return;
      }
      addWeaponItemInTable(itemRowBlock.dataset.id);
    }
  }
});
