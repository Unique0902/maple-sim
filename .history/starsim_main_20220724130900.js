...
additionBoxDescriptionBlock.addEventListener('mousedown', (e) => {
  ...
  additionBoxDescriptionBlock.addEventListener(
    ...
    (additionMouseMoveEvent = (e) => {
      const x = e.clientX;
      const y = e.clientY;
      const xDiff = x - firstCurXLoc;
      const yDiff = y - firstCurYLoc;
      const left = reinforceAdditionBox.getBoundingClientRect().left;
      const top = reinforceAdditionBox.getBoundingClientRect().top;
      reinforceAdditionBox.style.left = `${left + xDiff}px`;
      reinforceAdditionBox.style.top = `${top + yDiff}px`;
    })
  );
});
