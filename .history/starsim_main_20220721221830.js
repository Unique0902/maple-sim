...
let recentReinforceMinute;

function checkStarcatchLevel(userItem) {
  const nowTime = new Date();
  if (recentReinforceMinute) {
    const diffSec = Math.floor(
      (nowTime.getTime() - recentReinforceMinute.getTime()) / 1000
    );
    if (diffSec >= 2400) {
      userItem.starcatchLevel = 1;
      userItem.clearStarforceNum();
    } else if (diffSec >= 600) {
      const diffTenMin = Math.floor(diffSec / 600);
      for (let i = 0; i < diffTenMin; i++) {
