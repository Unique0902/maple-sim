...
function calculateChanceTime(result, userItem) {
  ...
  return userItem.returnChanceNum();
}

function checkChanceTime(result, userItem) {
  const chanceNum = calculateChanceTime(result, userItem);
  if (chanceNum >= 2) {
...
reinforceAdditionBoxConfirmBtn.addEventListener('click', () => {
  ...
  isReinforcing = true;
  setTimeout(() => {
    checkChanceTime(result, userItem);
