...
function moveFollowedItemBox(e) {
  followedItemBox.style.left = `${e.xpos - 15}px`;
  followedItemBox.style.top = `${e.ypos - 15}px`;
}
function controlFollowedItemBox(xpos, ypos) {
...
itemWindowTable.addEventListener('click', (e) => {
  if (e.target.className === 'table__img') {
    ...
    showFollowedItemBox();
    controlFollowedItemBox(e.pageX, e.pageY);
    document.addEventListener('mousemove', moveFollowedItemBox, true);
    document.addEventListener('click', (event) => {
      console.log(event.target.tagName);
      if (event.target.tagName === 'TD') {
        console.log('hi');
        e.target.innerHTML = '';
        document.removeEventListener('mousemove', moveFollowedItemBox, true);
      }
    });
    hideSearchedPage();
  }
