...
function disposeStarcatch(userItem) {
  if (userItem.checkChanceTime === true) {
    return false;
  }
  if (starcatchCheckBox.checked) {
    userItem.isStarcatch = true;
    return startStarcatch();
  } else {
    return false;
  }
}
...
reinforceAdditionBoxConfirmBtn.addEventListener('click', () => {
  ...
  const starcatchResult = disposeStarcatch(userItem);
  if (userItem.isStarcatch === true) {
  } else {
  }
  const result = reinforceStarforce(userItem, starcatchResult);
