...
additionBoxDescriptionBlock.addEventListener('mousedown', (e) => {
  ...
  additionBoxLeft = additionRect.left;
  additionBoxTop = additionRect.top;
  document.removeEventListener('mousemove', additionMouseMoveEvent);
  document.addEventListener(
    'mousemove',
    (additionMouseMoveEvent = (e) => {
      console.log(firstCurXLoc, firstCurYLoc);
      const x = e.clientX;
      const y = e.clientY;
      const xDiff = firstCurXLoc - additionBoxLeft;
      const yDiff = firstCurYLoc - additionBoxTop;
      reinforceAdditionBox.style.transform = `translate(${x - xDiff}px,${
        y - yDiff - additionBoxTop
      }px)`;
    })
  );
});
